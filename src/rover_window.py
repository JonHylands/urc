...
class RoverWindow:
    ...
    def build_root_window(self):
        ...
        image = VisualJpgImage(box, 'Crawler-240.jpg')
...
class RoverChooseWindow:
    ...
    def build_rover_choose_window(self):
        ...
        label = VisualLabel(Point(0, 20), 'CRAWLER', font_32, True, Color.TITLE)
...
class RoverMissionWindow:
    ...
    def build_window_for_mission(self, mission_name):
        ...
        label = VisualLabel(Point(0, 0), 'Mission: {}'.format(mission_name), font_20, True, Color.LABEL)
        ...
        button_y += 40
        button = VisualButton(Rectangle(Point(0, button_y), Point(160, 35)), "SET MODE", font_25, Color.BUTTON)
        middle_view.add_component(button)
        button.register_click_handler(self.clicked_set_mode, mission_name)
    ...
    def clicked_set_mode(self, mission_name):
        print('Edit Mission: {}'.format(mission_name))
        RoverSetModeWindow(self.window_manager, self.display, self.set_mode)
...
class RoverSetModeWindow:
    def __init__(self, window_manager, display, mode_callback):
        self.window_manager = window_manager
        self.display = display
        self.mode = 'RC'
        self.mode_callback = mode_callback
        self.build_rover_set_mode_window()
        self.window_manager.push_window(self.choose_window)

    def build_rover_set_mode_window(self):
        self.choose_window = Window(self.display, 'Rover Set Mode')
        ...
        self.mode_label = VisualLabel(Rectangle(Point(0, 10), Point(160, 30)), 'MODE: {}'.format(self.mode), font_25, True, Color.TITLE, Color.BACKGROUND)
        middle_view.add_component(self.mode_label)

        button = VisualButton(Rectangle(Point(0, 60), Point(160, 35)), "RC MODE", font_25, Color.BUTTON)
        middle_view.add_component(button)
        button.register_click_handler(self.clicked_set_rc_mode)

        button = VisualButton(Rectangle(Point(0, 100), Point(160, 35)), "AUTO MODE", font_25, Color.BUTTON)
        middle_view.add_component(button)
        button.register_click_handler(self.clicked_set_auto_mode)

        self.choose_window.register_about_to_close(self.about_to_close)

    def clicked_set_rc_mode(self):
        self.mode = 'RC'
        self.mode_label.set_text('MODE: {}'.format(self.mode))

    def clicked_set_auto_mode(self):
        self.mode = 'AUTO'
        self.mode_label.set_text('MODE: {}'.format(self.mode))

    def about_to_close(self, window):
        self.mode_callback(self.mode)

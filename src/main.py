...
from balance_window import BalanceDroidWindow
...
class Failsafe:
    def __init__(self):
        print('Crawler Failsafe Handheld by Jon Hylands')
        self.display = Display()
        print('Display initialized')

        self.display.jpg('Crawler-240.jpg', 0, 0)
        print('JPG Displayed')
        time.sleep_ms(1000)
        ...
        self.balance = BalanceDroidWindow(self.window_manager, self.display)
        self.robot_chain = WindowChain('Robots', [self.rover.root_window, self.balance.root_window, self.roz.root_window, self.ttrobot.root_window])
